...
; expect answer-to-everything

(define one 1)
; expect one

(/ answer-to-everything one)
; expect 42
...
(define (list-partitions total max-pieces max-value)
    ; *** YOUR CODE HERE ***
    (define (recursive-partition total-remaining pieces-remaining max-value partition-list)
        (cond
            ;Finish cases
            ((= total-remaining 0)
                (list partition-list))
            ...
            (else
                (append
                    ;Recursive call for partitions with a max-value partition
                    (recursive-partition
                        (- total-remaining max-value)
                        (- pieces-remaining 1)
                        max-value
                        (append partition-list (list max-value))
                    )
                    ;Recursive call for smaller valued partitions
                    (recursive-partition
                        total-remaining
                        pieces-remaining
                        (- max-value 1)
    ...
    (recursive-partition total max-pieces max-value nil)
)
...
(define (tree-sum-list tree)
    ; *** YOUR CODE HERE ***
    (define (recursive-tree-sum-list child-list sum-so-far output-list)
        (if (null? child-list)
            nil
            (append
                (if (null? (children (car child-list)))
                    (list (+ sum-so-far (entry (car child-list))))
                    (recursive-tree-sum-list (children (car child-list)) (+ sum-so-far (entry (car child-list))) output-list)
                )
                (recursive-tree-sum-list (cdr child-list) sum-so-far output-list))
        )
    )
    (recursive-tree-sum-list (list tree) 0 nil)
)

(define tree2 (make_tree 42 nil))
(tree-sum-list tree2)
; expect (42)
