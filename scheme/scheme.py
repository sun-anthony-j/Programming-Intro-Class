...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("Wrong argument type for function")
...
def do_lambda_form(vals, env):
    ...
    if len(vals.second) > 1:
        ...
        return LambdaProcedure(formals, vals.second[0], env)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if type(target[0]) == int:
            raise SchemeError("Variable can't start with an number")
...
def do_let_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    for name_value_bind in bindings:
        names = Pair(name_value_bind[0], names)
        values = Pair(scheme_eval(name_value_bind[1], env), values)
...
def do_and_form(vals, env):
    ...
    for parameter in vals:
        if parameter is vals[len(vals) - 1]:
            return vals[len(vals) - 1]
        if scheme_false(scheme_eval(parameter, env)):
            return False
...
def do_or_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    for parameter in vals:
        if scheme_true(scheme_eval(parameter, env)):
            return parameter
    return False
...
def check_formals(formals):
    ...
    for symbol in formals:
        if not scheme_symbolp(symbol):
            raise SchemeError('Invalid symbol' + str(symbol))
        if symbol in symbol_list:
            raise SchemeError('Repeated symbol:'+ str(symbol))
        symbol_list.append(symbol)
    if not scheme_listp(formals):
        raise SchemeError('Not a well formed list:' + formals)
