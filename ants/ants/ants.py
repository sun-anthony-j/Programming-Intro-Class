...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            assert self.ant is None or self.ant.can_contain(insect) or insect.can_contain(self.ant), 'Two ants in {0}'.format(self)
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        "*** YOUR CODE HERE ***"
        if self.place.ant is not None and self.place.ant.name=='Ninja':
            return False
        else:
...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        if self.container is True and ant.container is False and self.ant is None:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        target = self.place
        distance = 0
        while target.name != hive.name:
            if target == None:
                return None
            elif ((distance <= self.max_range) and (distance >= self.min_range) and (len(target.bees)>0)):
                return random_or_none(target.bees)
            else:
                distance += 1
...
class PrincessAnt(Ant):
    ...
    def __init__(self):
        "*** YOUR CODE HERE ***"
        Ant.__init__(self)
        self.armor_boost = True

    def action(self, colony):
        """If this PrincessAnt has never taken an action before, it should add 1
        point of armor to all other Ants in the tunnel."""
        "*** YOUR CODE HERE ***"
        if self.armor_boost == True:
            if self.place.ant.container is True:
                self.place.ant.armor+=1
            left=self.place.exit
            while left is not colony.queen:
                if left.ant is not None:
                    left.ant.armor+=1
                left=left.exit
            right=self.place.entrance
            while right is not colony.hive:
                if right.ant is not None:
                    right.ant.armor += 1
                right=right.entrance
            self.armor_boost = False
...
def apply_effect(effect, bee, duration):
    ...
    effect_method=effect(bee.action)
    old_action=bee.action
    def new_action(colony):
        nonlocal duration_timer
        if duration_timer > 0:
            duration_timer -= 1
            effect_method(colony)
        else:
            old_action(colony)
