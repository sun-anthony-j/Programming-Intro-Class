...
class Mobile(object):
    ...
    def isbalanced(self):
        ...
        "*** YOUR CODE HERE ***"
        return (self.left.torque == self.right.torque and self.left.contents.isbalanced and self.right.contents.isbalanced)
...
def interpret_mobile(s):
    ...
    for t in ('Branch', 'Weight', 'Mobile'):
        "*** YOUR CODE HERE ***"
        temp=interpret_mobile(s.replace("T",t,1))
        if temp is not None:
            return temp
...
def iterator_to_stream(iterator):
    ...
    def output_stream():
        try:
            ...
            return Stream(next(a), compute_rest)
