...
def tokenize(line):
    ...
    """
    "*** YOUR CODE HERE ***"
    for key in list(ALL_BRACKETS):
        replacement=' '+key+' '
        line=line.replace(key, (' '+key+' '))
    output=line.split()
    for element in output:
        if not(element in ALL_BRACKETS):
            if coerce_to_number(element)is not None:
                element=coerce_to_number(element)
            else:
                raise ValueError('invalid token '+ element)
...
def isvalid(tokens):
    ...
    """
    "*** YOUR CODE HERE ***"
    if tokens[0] in list(LEFT_RIGHT.keys):
        return isvalid(tokens[1:LEFT_RIGHT[tokens[0]]])
