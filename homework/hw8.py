...
def mutate_rational(r):
    ...
    """
    "*** YOUR CODE HERE ***"
    super(Rational,r).__setattr__('denominator', r.denominator+1)
...
def apply(operator_name, shape):
    ...
    def squarePerimeter(s):
        return s.side*4
    def rectanglePerimeter(r):
        return 2*r.width+2*r.height
    area={'s':squareArea,'r':rectangleArea}
    perimeter={'s':squarePerimeter, 'r':rectanglePerimeter}
    operator={'area':area,'perimeter':perimeter}
    return operator[operator_name][type_tag(shape)](shape)
