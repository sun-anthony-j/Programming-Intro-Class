...
def depth(s, v):
    ...
    "*** YOUR CODE HERE ***"
    output=0
    while set_contains3(s.left,v)or set_contains3(s.right,v):
        if set_contains3(s.left,v):
            s=s.left
        elif set_contains3(s.right,v):
            s=s.right
        output+=1
    return output
...
def tree_to_ordered_sequence(s):
    ...
    output=Rlist(s.entry)
    if s.left is not None:
        output=union_set2(output,tree_to_ordered_sequence(s.left))
    if s.right is not None:
        output=union_set2(output,tree_to_ordered_sequence(s.right))
...
def partial_tree(s, n):
    ...
    if n==1:
        ...
        return Tree(restof(s,(n-1)//2).first ,partial_tree(s, left_size)[0],partial_tree(restof(s,(n-1)//2+1),right_size)[0]),restof(s,n)
